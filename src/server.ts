...
export default class Server implements Party.Server {
	...
	static async onFetch(
		...
		if (path.startsWith('/roles/') && req.method === 'GET')
			return await roles(req, path, tokenData, env)

		if (path.startsWith('/channels/') && req.method === 'GET')
			return await channels(req, path, tokenData, env)

		if (path.startsWith('/members/') && req.method === 'GET')
			return await members(req, path, tokenData, env)

		if (/\/projects\/.+\/tasks\/.+\/notify\/create/.test(path) && req.method === 'POST')
			return await notifyCreate(req, path, tokenData, env)
	}
}
